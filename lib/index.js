...
export const apply = (ctx, rawConfig) => {
  ...
  if (!config.token) {
    ctx.logger.warn(
      'dsh-wechat: 未配置 token，回调会拒绝所有请求。' +
      '请在 profile 的 cordis.patch.yml 里为本插件补上 config.token。',
    );
  }
  if (config.allowFrom.length === 0) {
    ctx.logger.warn(
      'dsh-wechat: allowFrom 白名单为空，任何人都无法下指令。' +
      '关注测试号后插件会回信告诉对方 openid，填进来即可。',
    );
  }
  if (!api.configured) {
    ctx.logger.warn(
      'dsh-wechat: 未配置 appId / appsecret，结果无法异步推回微信，' +
  ...
  ctx.inject(['agents', 'workspaceRegistry', 'agentPresets', 'permissionPresets', 'agentDefaultModel'], (agentCtx) => {
    ...
    const openAgent = async ({ sessionId, meta, agentOptions, setup }) => {
      ...
      if (knownExists) {
        ...
        ctx.logger.info(`dsh-wechat: 复用已有会话 ${sessionId}`);
        return handle;
      }
      ...
      try {
        ...
        if (/already exists/i.test(String(err.message))) {
          ctx.logger.info(`dsh-wechat: create 报告会话已存在，改为复用 ${sessionId}`);
          try {
            return await resume();
          } catch (resumeError) {
            throw new Error(
    ...
    ctx.logger.info(
      `dsh-wechat: 会话运行时已就绪（工作区 ${workspacePath}` +
      `${config.agentPreset ? `，preset ${config.agentPreset}` : ''}）`,
    );
    runtimeError = null;

    ctx.effect(() => () => bridge?.disposeAll(), 'dsh-wechat: sessions');
  });
  ...
  async function runAndPush(message) {
    ...
    try {
      ...
      if (!push.ok) {
        ...
        ctx.logger.warn(`dsh-wechat: 结果推送失败：${push.error}`);
      } else {
        recordEvent('push-ok', `${push.sent} 条`);
        ctx.logger.info(
          `dsh-wechat: 已回推结果给 ${message.from}（${push.sent} 条，` +
          `总耗时 ${Math.round((Date.now() - started) / 1000)} 秒）`,
        );
      }
    } catch (err) {
      recordEvent('turn-threw', err.message);
      ctx.logger.warn(`dsh-wechat: 后台任务异常：${err.message}`);
  ...
  ctx.effect(() => {
    ...
    ctx.logger.info(
      `dsh-wechat: 回调已挂载 → 本机 http://127.0.0.1:${ctx.webServer.port}${path}` +
